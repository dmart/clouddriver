...
public class NetflixSTSAssumeRoleSessionCredentialsProvider
  ...
  public NetflixSTSAssumeRoleSessionCredentialsProvider(
    ...
    var stsClientBuilder = AWSSecurityTokenServiceClient.builder()
      .withCredentials(longLivedCredentialsProvider)
      .withRegion(region);

    if (roleArn.contains("aws-us-gov")) {
      stsClientBuilder.withEndpointConfiguration(
        new AwsClientBuilder.EndpointConfiguration("sts.us-gov-west-1.amazonaws.com", region));
    } else if (roleArn.contains("aws-cn")) {
      stsClientBuilder.withEndpointConfiguration(
        new AwsClientBuilder.EndpointConfiguration("sts.cn-north-1.amazonaws.com.cn", region));
    }
